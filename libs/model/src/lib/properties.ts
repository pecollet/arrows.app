...
export const summarizeProperties = (selectedEntities:Entity[], graph:Graph) => {
  const keys:{key:string, nodeCount:number}[] = [], 
        values = new Map<string, {value:string, inSelection:boolean, nodeCount:number}[]>()
...
export const setArrowsProperty = (entity:Entity, key:string, value:string) => {
